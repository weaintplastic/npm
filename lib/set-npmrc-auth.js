...
module.exports = async (
  ...
  console.log("Verify authentication for registry %s", registry);
  ...
  if (configs) {
    console.log("Reading npm config from %s", configs.join(", "));
  }
  ...
  if (NPM_USERNAME && NPM_PASSWORD && NPM_EMAIL) {
    ...
    console.log(`Wrote NPM_USERNAME, NPM_PASSWORD and NPM_EMAIL to ${npmrc}`);
  } else if (NPM_TOKEN) {
    await outputFile(
      npmrc,
      `${currentConfig ? `${currentConfig}\n` : ""}${nerfDart(registry)}:_authToken = \${NPM_TOKEN}`
    );
    console.log(`Wrote NPM_TOKEN to ${npmrc}`);
  } else {
